...
export default class Schema {
  ...
  version(version) {
    if (!arguments.length) return parseInt(Object.keys(this._versions).sort().pop() || 0, 10)
  ...
  upgrade(dbName, version, keepOpen) {
    ...
    function thenableUpgradeVersion(dbLast, res, rej, start) {
      ...
      while (!versionIter.done && versionIter.value <= lastVers) {
    ...
    afterOpen = (db, res, rej, start) => {
      ...
      const processReject = (err, callbackIndex) => {
        ...
        err.retry = () => {
          return new Promise((resolv, rejct) => {
            ...
            open(dbName).catch(rejct).then((dbs) => {
              setVersions()
      ...
      promise.then(() => thenableUpgradeVersion(db, res, rej))
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      open(dbName, upgradeneeded((e, ...dbInfo) => {
        // Upgrade from 0 to version 1
        const versionIter = versions.next()
        if (versionIter.done) {
          reject(new Error('No schema versions added for upgrade'))
          return
        }
        versionSchema = versionIter.value
        upgradeVersion(versionSchema, ...dbInfo)
